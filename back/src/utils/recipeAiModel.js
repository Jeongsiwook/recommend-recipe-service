...
async function recipeAiModel(title, ingredients) {
    const result = "시금치는 데쳐서 물기를 뺀다. 계란은 완숙으로 삶아서 식힌다. 밥을 지을 때 밥은 찬밥을 사용해도 되고 대파를 넣으면 조금 더 아삭한 걸 맛볼 수 있다. 밥 위에 시금치 비빔밥을 올린다.";
    return result;
    ...
    // const result = await session.run([tensorX, tensorY, tensorZ]);
    // // const result = await session.run([tensorX, tensorY, tensorZ]);
...
async function recipeAiModel(name, ingredients) {
  ...
  console.log(
    `Got an Tensor of size ${outputData.data.length} with all elements being ${outputData.data[0]}`
  );
}
